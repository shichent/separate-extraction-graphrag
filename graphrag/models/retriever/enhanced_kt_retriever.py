...
class KTRetriever:
    def __init__(
                    ...
                    for line in f:
                        ...
                        if line and "\t" in line:
                            ...
                            if len(parts) == 2 and parts[0].startswith("id: ") and parts[1].startswith("Chunk: "):
                                ...
                                fail_counter += 1
                                if fail_counter <= 5:
                                    logger.warning(f"Warning: Failed to parse line: {len(parts)} parts, 0: {parts[0][:10]}, 1: {parts[1][:10]}")
        ...
        self.use_exact_keyword_matching = False  # Set to False for original substring matching
    ...
    def _process_chunk_results(self, chunk_results: Dict, question_embed: torch.Tensor, top_k: int) -> Tuple[List[str], set]:
        ...
        for chunk_id in chunk_ids:
            ...
            content = chunk_contents.get(chunk_id, f"[Missing Content for chunk ID: {chunk_id}]")
    ...
    def _get_matching_chunks(self, chunk_ids: set) -> Dict[str, str]:
        """Get chunk contents for given chunk IDs."""
        fail_counter = 0
        for chunk_id in chunk_ids:
            if chunk_id not in self.chunk2id and fail_counter < 5:
                fail_counter += 1
                logger.warning(f"Chunk ID [{chunk_id}] not found in chunk2id mapping.")
        if fail_counter >= 5:
    ...
    def _extract_query_keywords(self, question: str, method: str = "gliner") -> List[str]:
        """
        ...
        Args:
            ...
            method: Keyword extraction method, can be "spacy" or "gliner" (default: "spacy")
        ...
        if method == "spacy":
            ...
            for entity in entities:
                ans.append(re.sub(r'(?<=[\u0080-\uFFFF]) (?=[\u0080-\uFFFF])', '', entity["text"]))
            unique_keywords = list(set(ans))
        logger.info(f"Keywords extracted from question: {unique_keywords}")
        ...
        try:
            ...
            for i, (score, idx) in enumerate(zip(scores[0], indices[0])):
                if idx != -1 and idx in self.index_to_chunk_id:
                    ...
                    similarity_scores[chunk_id] = score
                    
                    if chunk_id in self.chunk2id:
                        chunk_contents[chunk_id] = self.chunk2id[chunk_id]
                    else:
                        chunk_contents[chunk_id] = f"[Missing content for chunk {chunk_id}]"
                        logger.warning(f"Can't find [{chunk_id}] in _chunk_embedding_retrieval")
        ...
        try:
            ...
            for chunk_id, content in chunk_contents.items():
                try:
                    ...
                    faiss_score = original_scores[chunk_id] if chunk_id in original_scores else 0.0
                    combined_score = (faiss_score + similarity) / 2.0  # Average of both scores
                    
                    chunk_similarities.append((chunk_id, content, combined_score))
                    
                except Exception as e:
                    logger.error(f"Error calculating similarity for chunk {chunk_id}: {str(e)}")
                    faiss_score = original_scores[chunk_id] if chunk_id in original_scores else 0.0
                    chunk_similarities.append((chunk_id, content, faiss_score))
            
            chunk_similarities.sort(key=lambda x: x[2], reverse=True)
            
            top_chunks = chunk_similarities[:top_k]
            
            reranked_chunk_ids = [chunk_id for chunk_id, _, _ in top_chunks]
            reranked_scores = {chunk_id : score for chunk_id, _, score in top_chunks}
            reranked_contents = {chunk_id : content for chunk_id, content, _ in top_chunks}
